...
**身分只來自完整 id，不來自內容比對。** verify 三輪各找到一種錯接，全是 exit 0、無警告：(1) 片段抽取失敗後退化成 ordinal；(2) 短前綴加截斷片段，原命題改寫後另一筆開頭相同的命題被當成它；(3) 短前綴加全文相等，原命題改寫後兄弟命題繼承了舊文字。三者同源——內容說的是命題**現在**寫什麼，不是審查者**當時**走的是哪一筆。所以：

- proofread checklist 模板改為寫**完整 UUID**（SKILL.md 一行）。
- 完整 id 直接對到該命題，其文字仍須以片段開頭（擋改寫後的舊判定）。
- 短前綴一律不解析，提示以完整 id 重產；`--allow-unmatched` 可略過並列出。
- view ordinal 不用。
...
- [uuid_short 前綴在 UUIDv7 帳冊上幾乎必撞、截斷片段不構成身分] → checklist 改寫完整 id；短前綴一律不解析，永不以內容猜身分。
